...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `You are "Ask Docker", a senior Docker engineer and trainer.
...
- When a command is DESTRUCTIVE (rm -f, system prune, volume rm, image prune -a, container prune, kill, down -v, network prune), prefix the explanation line with "⚠️ DESTRUCTIVE:" and warn what data is lost.
- Prefer official Docker CLI (docker, docker compose). Note OS/permission caveats when relevant.
- Keep answers focused. No filler.`;
    ...
    if (!response.ok) {
      ...
      console.error("docker-ask AI error:", response.status, t);
    ...
    console.error("docker-ask error:", e);
